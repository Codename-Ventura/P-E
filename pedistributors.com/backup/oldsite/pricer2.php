...
<div style="float:right"><a href="test.php"><img src="images/pricer/viewdb.jpg" border="0"></a></div>
...
$current_page = "1";
while($current_page <= $_POST['pages']){
...
foreach ($test as $div){
	...
	*/
	...
	$price = trim($price);
	echo $price; echo "</td>";
	$checkrows = mysql_num_rows(mysql_query("SELECT * FROM pricer WHERE partnum = '$partnumber'"));
	if (isset($_POST['addtodb'])){
	
	
	
	if($checkrows == "0"){
			mysql_query("INSERT INTO pricer (partnum, price) VALUES ('$partnumber', '$price')");
			echo "<td bgcolor=\"#BCE8EB\">Data Added</td>";
			}
	elseif(mysql_num_rows(mysql_query("SELECT * FROM pricer WHERE partnum = '$partnumber' AND price = '$price'")) == "0"){
			mysql_query("UPDATE pricer SET price = '$price' WHERE partnum = '$partnumber'");
