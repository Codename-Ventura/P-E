...
foreach ($test as $div){
	...
	*/
	...
	if($checkrows == "0"){
			mysql_query("INSERT INTO pricer3 (line, partnum, price, sort_num) VALUES ('$line','$partnumber', '$price', '$sort')");
	...
	elseif(mysql_num_rows(mysql_query("SELECT * FROM pricer3 WHERE partnum = '$partnumber' AND price = '$price' AND line = '$line'")) == "0"){
			mysql_query("UPDATE pricer3 SET price = '$price', sort_num = '$sort', WHERE partnum = '$partnumber' AND line = '$line'");
			echo "<td bgcolor=\"#FFFFCC\">Data Updated ".$sort."</td>";
			$sort++;
			}
	else{ echo "<td bgcolor=\"#9AE7A8\">Pricing Up to Date ".$sort."</td>";
	mysql_query("UPDATE pricer3 SET sort_num = '$sort', WHERE partnum = '$partnumber' AND line = '$line'");
	$sort++;
			}
