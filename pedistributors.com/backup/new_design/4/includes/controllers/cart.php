...
function addtocart($parts){
	...
	foreach($parts as $k=>$v){
		if($_SESSION['last_search_results'][$k][7] == "0"){
			$this->pageResponse("out_of_stock", $k);
			unset($parts[$k]);
		}
		elseif($v > $_SESSION['last_search_results'][$k][7]){
			$this->pageResponse("qty_exceeded", $k);
			$parts[$k] = $_SESSION['last_search_results'][$k];
			$parts[$k]['quantity'] = $_SESSION['last_search_results'][$k][7];
		}
		else{
			$parts[$k] = $_SESSION['last_search_results'][$k];
			$parts[$k]['quantity'] = $v;
...
function modifycart($parts){
	foreach($parts as $k=>$v){
		...
		elseif($v <= $this->cart[$k][7] && $v !== $this->cart[$k]['quantity']){
