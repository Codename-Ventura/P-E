...
<?php
if(!empty($_REQUEST['sort_big'])){
$sort = $_REQUEST['sort_big'];
}
else{ 
$sort = "0";
}
...
<br><input type="text" size="4" name="sort_big" style="margin-left:10em;">Enter next sort num if querying for same linecode
</td>
</tr>
<tr bgcolor="#deeaff" style="font-size:9px; text-align:center"><td><p>Enter Page Numbers Below (for large queries)</p></td></tr>
<tr bgcolor="#deeaff" style="display:table-cell; vertical-align:middle; text-align:center"><td>Start Page:<input type="text" size="2" name="start_pg"> End Page:<input type="text" size="2" name="end_pg"> 
  
    <br><label for="Change">Define Price Match (rounds up to .99 by default)</label>
    <select name="Change" id="Change">
      <option value="99" <?php if (!(strcmp(99, "$pset"))) {echo "selected=\"selected\"";} ?>>.99</option>
      <option value="match" <?php if (!(strcmp("match", "$pset"))) {echo "selected=\"selected\"";} ?>>Match</option>
      <option value="50" <?php if (!(strcmp(50, "$pset"))) {echo "selected=\"selected\"";} ?>>Undercut by .50</option>
    </select>
    <label for="Actual">    </label></td></tr>
...
if ($pset == '99' && $price[0] > 2){
	$price[1] = '99';}
 if($pset == "match"){
		$price[1] = $price[1];}
	if($pset == '50' && $price[0] >1 && $price[1] >50){
		$price[1] = ($price[1] - 50);
		echo 'wtf';}
		elseif($pset == '50' && $price[1] <50){
			$price[1] ='00';}
	if ($price[1] == "9" || $price[1] == "8" || $price[1] == "7" || $price[1] == "6" || $price[1] == "5" || $price[1] == "4" || $price[1] == "3" || $price[1] == "2" || $price[1] == "1"){
	$price[1] = "0".$price[1];}	
