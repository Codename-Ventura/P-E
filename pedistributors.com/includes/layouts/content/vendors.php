...
$image_table .= "<td align=\"center\" onclick=\"location.href='index.php?p=vendors&vendor_detail=".$v[id]."'\" onMouseOver=\"this.style.border='1px #000 dashed'\" onMouseOut=\"this.style.border='1px #FFF dashed'\" style=\"border:1px #FFF solid; text-align:center\" class=\"image_table_cell\"><a href=\"index.php?p=vendors&vendor_detail=".$v[id]."\"><img src=\"".$v[logo]."\" width=\"150px\" style=\"padding:.25em\" align=\"center\" border=\"0\"></a></td>";
$count++;
}
...
?>
<div class="divider">March Mayhem Featured Vendors</div>
...
<?php } ?>
<?php
if($main == "vendors" && !isset($_GET[vendor_detail])){
...
<div class="main">
<?php if($vendor_details[LandingPage] != ''){ ?>
<iframe src="<?php echo $vendor_details[LandingPage]?>" width="780" height="<?php if($vendor_details[lpheight] != ''){ echo $vendor_details[lpheight]; }else{?>2000<?php } ?>" scrolling="no"></iframe> <?php } 
else{?>
	<div class="text-box">
    ...
    <div style="float:left; margin-left:.5em">
        ...
        <?php
			...
			if($vendor_details[vid] !== ""){?>
		<div style="clear:left; font-size:14px"><center><br /><?php echo $vendor_details[vid];?></center>
            </div>
          	<?php }?>
</div>
...
$vendor_details[products] = array_slice($vendor_details[products],0,1);
foreach($vendor_details[products] as $k=>$v){
?>
<?php //include ('/includes/controllers/pagination.php');
?>
<div style="float:left; width:790px; margin:1em; padding:.5em">
    ...
    <div style="float:left; width:70%"><a href="http://www.pedistributors.com/index.php?p=search&linecode=<?php echo $v[line];?>&partnumber=<?php echo $v[partnum];?>" target="_blank">Click here to buy now!</a></div>
    </div>
</div>
