...
class Login extends Index{
	...
	function Login(){
		...
		if(isset($_POST['reminder'])){
			$this->emailPassword($_POST[customer_number]);
		}
	}
	...
	function checkSql($username, $password){
		...
		function checkSqlUser($username){

		include('includes/db.php');
		$query = mysql_fetch_array(mysql_query("SELECT * FROM `login` WHERE `customer_number` = '$username'"));
		$query2 = mysql_fetch_array(mysql_query("SELECT * FROM `login` WHERE `email` = '$username'"));
		if(!$query && !$query2){
			return FALSE;
		}
			if($_POST[customer_number] == $query[customer_number]){
				return $this->SqlUserData = $query;
			}
			if($_POST[customer_number] == $query2[email]){
				return $this->SqlUserData = $query2;
			}
		}
	...
	function emailPassword($custNumber){
		if(!$this->checkSqlUser($custNumber)){
			$this->pageResponse("no_email");
		}else{
		...
		$to = $this->SqlUserData[email];
		$subject = "Online Ordering Login Information";
		$body = "Login Number:".$this->SqlUserData[customer_number]."  Password: ".$this->SqlUserData[password]."\nPlease note your password and delete this email.";
		...
		$this->pageResponse("password_sent");			
...
if(isset($_SESSION['loggedin']) & !isset($_SESSION['returnu'])){
	header('Location: index.php?p=members');
}
if(isset($_SESSION['loggedin']) & isset($_SESSION['returnu'])){
	header('Location: index.php?'.$_SESSION['returnu'].'');
}
?>
