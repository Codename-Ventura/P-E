...
class Checkout extends Index {
	...
	parent::Index();
		...
		if($_SERVER['REMOTE_ADDR'] !== '192.168.1.1fff50'){

		if($this->placeOrder()){			
			$_SESSION['sitemessage'] = "order_success";			
			$this->logOrder();
			unset($_SESSION['shopping_cart']);
			unset($_SESSION['cart_total']);
			header('Location: index.php?p=details&order='.$this->orderNumber.'');
	...
	function OrderTransaction(){	
		...
		$this->queryText .= $this->formatQueryLine(array('HD', $_SESSION['customer']['name']));
		$this->queryText .= $this->formatQueryLine(array('PO', $_POST['po_number']));
		$this->queryText .= $this->formatQueryLine(array('C2', $_POST['notes']));
		$this->queryText .= $this->formatQueryLine(array('M', $_SESSION['customer']['name']));
	...
	function logOrder(){
		...
		$date = date("Y-m-d");
		$login_number = $_SESSION[customer][login];
		$cart_total = $_SESSION[cart_total];
		mysql_query("INSERT INTO `b2b_orderlog` (`date`, `cust_num`,`amount`) VALUES ('$date', '$login_number', '$cart_total')");
